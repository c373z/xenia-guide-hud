...
void VdSwap_entry(
  ...
  if (guide_draw_fn_ && guide_draw_this_) {
    ...
    if (gth && !in_guide_draw) {
      ...
      uint64_t gargs[] = {guide_draw_this_};
      {
        // State of the command-buffer block AT DRAW ENTRY. 81A01358 sets base
        // +2B48 and cursor +2B4C together; the emitter later finds +2B4C zero.
        // If the base survives and only the cursor is zero, something clears
        // it between here and 81A015B8. If the base is zero too, the block was
        // reinitialised or the draw is on a different device record.
        static std::atomic<uint32_t> dn{0};
        uint32_t di = ++dn;
        {
          auto* qm = kernel_state()->memory();
          auto qrd = [qm](uint32_t a) {
            return xe::load_and_swap<uint32_t>(qm->TranslateVirtual(a));
          };
          uint32_t qdc = qrd(guide_draw_this_ + 12);
          uint32_t qwrap = qdc ? qrd(qdc + 0x1CCu) : 0;
          uint32_t qdev = qwrap ? qrd(qwrap + 0x0Cu) : 0;
          if (qdev) {
            guide_resv_dev_ = qdev;
            guide_resv_pre_ = qrd(qdev + 0x30u);
            // +0x30/+0x34 are what 81A042E0 (the reservation) actually
            // allocates from - NOT the +0x2B48/2B4C/2B50 block that 81A01358
            // and guide_bind_cmdbuf_kb set up.
            ...
            XELOGI("CmdbufAtDraw #{}: dev={:08X} RESERVE cur[30]={:08X} "
                   ...
                   di, qdev, qrd(qdev + 0x30u), qrd(qdev + 0x34u),
                   qrd(qdev + 0x3A00u), qrd(qdev + 0x2B48u),
                   qrd(qdev + 0x2B4Cu), qrd(qdev + 0x2B50u));
          }
        }
      }
      // MOVED EARLIER. This used to sit ~250 lines further down, after
      // the device-init block. CmdbufAtDraw showed the whole command
      // buffer block cold at draw entry (base 0, cursor 0, limit a
      // leftover heap pointer) and the init running only afterwards -
      // by which time the emitter's caller had already snapshotted the
      // cursor to its stack. The 81A01638 fault is a read-before-init,
      // not a clear: nothing zeroes [dev+0x2B4C], it was simply never
      // set when it was read.
